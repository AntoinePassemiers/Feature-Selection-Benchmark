...
def run_method(splits, X, X_tilde, y, method_name, dataset_name, k, k2=None, h_scores=None):
    ...
    for i in range(len(splits)):
        ...
        if h_scores is not None:
            ...
            wrapper = NNwrapper.create(dataset_name, len(X[0]))
            ...
            fsnet = FSNet(Model(n_selected), X.shape[1], 30, n_selected)
            ...
            wrapper = NNwrapper.create(dataset_name, len(X[0]), arch='cancelout-sigmoid')
            wrapper.fit(X_train, y_train)
            y_train_hat = wrapper.predict(X_train)
            y_hat = wrapper.predict(X_test)
            ...
            wrapper = NNwrapper.create(dataset_name, len(X[0]), arch='cancelout-softmax')
            wrapper.fit(X_train, y_train)
            y_train_hat = wrapper.predict(X_train)
            y_hat = wrapper.predict(X_test)
            scores = wrapper.model.cancel_out.get_weights().data.numpy()
            ...
            wrapper = NNwrapper.create(dataset_name, len(X_train[0]), arch='deeppink')
